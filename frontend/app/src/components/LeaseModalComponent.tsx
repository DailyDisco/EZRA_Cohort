...
export const LeaseModalComponent = ({ visible, onClose, mode, selectedLease }: LeaseModalProps) => {
    ...
    const cancelLeaseMutation = useMutation({
        mutationFn: async (leaseId: number) => {
            ...
            const response = await fetch(`${API_URL}/admin/leases/cancel/${leaseId}`, {
                method: 'POST',
                headers: {
                    Authorization: `Bearer ${token}`,
                    'Content-Type': 'application/json',
    ...
    const addLeaseMutation = useMutation({
        mutationFn: async (values: any) => {
            ...
            const response = await fetch(`${API_URL}/admin/leases/create`, {
                method: 'POST',
                headers: {
                    Authorization: `Bearer ${token}`,
                    'Content-Type': 'application/json',
    ...
    const sendLeaseMutation = useMutation({
        mutationFn: async () => {
            ...
            const response = await fetch(`${API_URL}/admin/leases/send/${selectedLease.id}`, {
                method: 'POST',
                headers: {
                    Authorization: `Bearer ${token}`,
                    'Content-Type': 'application/json',
    ...
    const renewLeaseMutation = useMutation({
        mutationFn: async (values: any) => {
            ...
            const response = await fetch(`${API_URL}/admin/leases/renew`, {
                method: 'POST',
                headers: {
                    Authorization: `Bearer ${token}`,
                    'Content-Type': 'application/json',
    ...
    const amendLeaseMutation = useMutation({
        mutationFn: async (values: any) => {
            ...
            const propertyAddress = values.new_apartment_id ? values.new_property_address : values.property_address;
            ...
            const response = await fetch(`${API_URL}/admin/leases/amend`, {
                method: 'POST',
                headers: {
                    Authorization: `Bearer ${token}`,
                    'Content-Type': 'application/json',
                ...
                return (
                    <div className="text-center p-4">
                        ...
                        <p className="text-gray-500 mb-4">
                            {mode === 'add'
                                ? 'The lease has been created in draft status.'
                                : mode === 'send'
                                  ? 'The lease has been sent for signing.'
                                  : mode === 'renew'
    ...
    return (
        <Modal
            title={status === 'success' ? 'Success' : status === 'error' ? 'Error' : getModalTitle()}
            open={visible}
